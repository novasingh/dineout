        ...
        view.setOnTouchListener(new OnSwipeTouchListener(getContext()) {
            ...
            public void onLongClick() {
                ...
                if (dish.getStatus() > 0 && dish.getStatus() < 4 && dish.getPriority()==0) {
                    ...
                    quantityInput.setHint("Enter quantity (1-" + dish.getServings() + ")");

                    AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
                    builder.setView(quantityInput);
                    builder.setMessage("Select Dish Quantity")
                            .setPositiveButton("Confirm Re-Order", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    String quantityStr = quantityInput.getText().toString();
                                    try {
                                        int selectedQuantity = Integer.parseInt(quantityStr);
                                        if (selectedQuantity > 0 && selectedQuantity <= dish.getServings()) {
                                            count = selectedQuantity;
                                            // PASTE RE-ORDER LOGIC HERE
                                            ...
                                            Toast.makeText(getContext(), "Invalid quantity. Please enter a number between 1 and " + dish.getServings(), Toast.LENGTH_LONG).show();
                                        }
